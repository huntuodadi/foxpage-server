...
export class GetPageBuildVersionDetail extends BaseController {
  ...
  async index(@QueryParams() params: PageBuildVersionReq): Promise<ResData<PageBuildVersion>> {
    try {
      ...
      let versionDetail = await this.service.version.info.getMaxContentVersionDetail(params.id, true);

      // The latest version is released or deleted, then create a new version
      if (!versionDetail || versionDetail.status !== VERSION_STATUS_BASE || versionDetail.deleted) {
        versionDetail = await this.service.version.info.createNewContentVersion(params.id);
      }
