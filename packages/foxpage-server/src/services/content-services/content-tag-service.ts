...
export class ContentTagService extends ContentServiceAbstract {
  ...
  async getAppContentByTags(params: AppTag): Promise<TagContentData[]> {
    ...
    const contentLiveInfo = await this.getAppContentLiveInfoByTags(fileDetail.id as string, params.tags);
  ...
  async getAppContentLiveInfoByTags(fileId: string, tags: Tag[]): Promise<ContentTagLiveVersion> {
    ...
    const matchTag = tag.matchContent(contentList as any[], tags) as any;
