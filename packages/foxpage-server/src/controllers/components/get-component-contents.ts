...
import { DELIVERY_REFERENCE } from '../../../config/constant';
...
export class GetComponentVersionDetail extends BaseController {
  ...
  async index(@QueryParams() params: ComponentFileContentReq): Promise<ResData<FileContentDetail>> {
    try {
      ...
      if (fileDetail.tags && fileDetail.tags?.[0]?.type === DELIVERY_REFERENCE) {
      ...
      const contentUserInfo = Object.assign(
        {
          ...
          liveVersion: this.service.version.number.getVersionFromNumber(
            contentDetail.liveVersionNumber,
          ) as string,
          online: !!(
            params.id === fileId &&
            onlineInfo &&
