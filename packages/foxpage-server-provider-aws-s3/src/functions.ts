...
export const downloadObjectContent = async (
  ...
  try {
    const data = await getObjectContent(client, bucketName, objectName);
    fs.writeFileSync(targetName, data);
...
export const bulkDownloadFiles = async (
  ...
  fileList.forEach((file) => {
    ...
    promises.push(
      limit(() =>
        downloadObjectContent(client, bucketName, file, tmpTarget + '/' + targetFilePath),
      ),
    );
  });
