...
export class SetContentVersionLive extends BaseController {
  ...
  async index(@Body() params: ContentLiveReq): Promise<ResData<Content>> {
    try {
      // Permission check
      const hasAuth = await this.service.auth.content(params.contentId);
      if (!hasAuth) {
      ...
      // Set live version
      this.service.content.info.updateContentItem(params.contentId, {
