...
export class FileInfoService extends FileServiceAbstract {
   ...
   * @returns File
   */
  create(params: Partial<File>): File {
    const fileDetail: File = {
      id: params.id || generationId(PRE_FILE),
    ...
    this.addDetailQuery(fileDetail);
    Service.logSingleton.getInstance().addLogItem(LOG_CREATE, fileDetail);
    return fileDetail;
  }
   ...
   * @returns Record
   */
  async addFileDetail(params: NewFileInfo): Promise<Record<string, number | (File & { contentId: string })>> {
    ...
    if ([PAGE_TYPE, TEMPLATE_TYPE].indexOf(params.type) === -1) {
   ...
   * @returns Promise
   */
  async updateFileDetail(params: AppTypeFileUpdate): Promise<Record<string, number>> {
    ...
    if (fileDetail.type === PAGE_TYPE) {
      ...
      if (params.tags && params.tags !== fileDetail.tags) {
        Service.logSingleton.getInstance().addLogItem(LOG_FILE_TAG, fileDetail);
      }
    }

    // Update file
    this.updateDetailQuery(params.id, _.pick(params, ['name', 'intro', 'type', 'tags']));

    // Update content name
    if ([VARIABLE_TYPE, CONDITION_TYPE].indexOf(fileDetail.type) !== -1) {
    ...
    // Save logs
    Service.logSingleton.getInstance().addLogItem(LOG_FILE_UPDATE, fileDetail);
   ...
   * @returns void
   */
  updateFileItem(id: string, params: Partial<File>): void {
    this.updateDetailQuery(id, params);
    Service.logSingleton.getInstance().addLogItem(LOG_FILE_UPDATE, Object.assign({ id }, params));
  }
   ...
   * @returns Promise
   */
  async setFileDeleteStatus(params: TypeStatus): Promise<Record<string, number>> {
    const fileDetail = await this.getDetailById(params.id);
    if (!fileDetail) {
    ...
    // Save logs
    Service.logSingleton.getInstance().addLogItem(LOG_FILE_REMOVE, [fileDetail]);
    Service.logSingleton.getInstance().addLogItem(LOG_CONTENT_REMOVE, contentVersion?.contentList || []);
   ...
   * @returns void
   */
  batchSetFileDeleteStatus(fileList: File[], status: boolean = true): void {
    this.setDeleteStatus(_.map(fileList, 'id'), status);
    Service.logSingleton.getInstance().addLogItem(LOG_FILE_REMOVE, fileList || []);
  }
   ...
   * @returns Promise
   */
  async getFileDetailByNames(params: FilePathSearch, createNew: boolean = false): Promise<Partial<File>> {
  ...
  createFileContentVersion(params: File, options: FileContentVersion): Record<string, string> {
    ...
    const contentVersionId = generationId(PRE_CONTENT_VERSION);
    ...
    this.create(params);
    Service.content.info.create(contentDetail);
    Service.version.info.create(contentVersionDetail);
  ...
  async copyFile(
    ...
    for (const file of fileList) {
      if (!options.relations[file.id]) {
        const newFileDetail = Service.file.info.create({
          ...
          intro: file.intro,
          suffix: file.suffix,
          tags: file.tags?.concat({ copyFrom: file.id }),
          type: file.type,
          folderId: options.folderId || '',
        });
        options.relations[file.id] = {
          newId:newFileDetail.id
        };
      }
    }
    ...
    for (const content of contentList) {
      !options.relations[content.id] && (tempRelations[content.id] = {
        newId: generationId(PRE_CONTENT_VERSION),
        oldName: content.title,
        newName: content.title + '_' + randStr(4),
      });
    }
