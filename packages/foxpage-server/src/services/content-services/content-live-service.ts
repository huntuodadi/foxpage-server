...
import { LOG_LIVE, VERSION_STATUS_RELEASE } from '../../../config/constant';
...
export class ContentLiveService extends ContentServiceAbstract {
   ...
   * @returns Promise
   */
  async setLiveVersion(params: LiveContentVersion): Promise<Record<string, number | string>> {
    ...
    if (versionDetail.status !== VERSION_STATUS_RELEASE) {
    ...
    if (result.code === 0) {
      this.setLiveContent(versionDetail.contentId, versionDetail.versionNumber, contentDetail);
      return { code: 0 };
    } else {
   ...
   * @returns void
   */
  setLiveContent(contentId: string, versionNumber: number, contentDetail?: Content): void {
    this.updateDetailQuery(contentId, { liveVersionNumber: versionNumber });

    Service.logSingleton.getInstance().addLogItem(LOG_LIVE, contentDetail || ({} as Content));
  }
