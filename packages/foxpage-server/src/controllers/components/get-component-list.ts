...
export class GetAppComponentList extends BaseController {
  ...
  async index(@Body() params: AppComponentsReq): Promise<ResData<ComponentContentInfo[]>> {
    try {
      const contentList = await this.service.content.component.getComponentVersionLiveDetails({
        applicationId: params.applicationId,
        type: (params.type as FileTypes[]) || COMPONENT_TYPE,
      ...
      const contentIds = _.pull(
        _.map(contentList, (content) => {
          return content?.package?.id;
        }),
        '',
        undefined,
      );
      const contentFileObject = await this.service.file.list.getContentFileByIds(contentIds as string[]);

      let components: ComponentContentInfo[] = [];
      for (let content of contentList as any[]) {
        ...
        componentCell.version = content.version as string;
