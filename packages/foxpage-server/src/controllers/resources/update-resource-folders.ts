...
export class UpdateResourceFolderDetail extends BaseController {
  ...
  async index(@Body() params: UpdateResourceFolderReq): Promise<ResData<Folder>> {
    try {
      ...
      this.service.folder.info.updateContentItem(params.id, {
        name: params.name,
        intro: params.intro || folderDetail.intro,
        folderPath,
      });
