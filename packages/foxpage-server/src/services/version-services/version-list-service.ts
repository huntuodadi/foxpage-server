...
import { VERSION_STATUS_BASE, VERSION_STATUS_DISCARD } from '../../../config/constant';
...
export class VersionListService extends VersionServiceAbstract {
  ...
  async getContentLiveOrBuildVersion(contentIds: string[]): Promise<ContentVersionNumber[]> {
    ...
    contentVersionList.forEach((version) => {
      if (!baseVersionInfoObject[version.contentId] && version.status === VERSION_STATUS_BASE) {
        baseVersionInfoObject[version.contentId] = _.pick(version, ['contentId', 'versionNumber']);
      } else if (!maxVersionInfoObject[version.contentId] && version.status !== VERSION_STATUS_DISCARD) {
