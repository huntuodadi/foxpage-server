...
export class FolderListService extends FolderServiceAbstract {
  ...
  async getPageChildrenList(
    ...
    const [applicationInfo, childFolderFiles] = await Promise.all([
      ...
      this.getAllChildrenRecursive([folderId], 1, false, fileTypes),
    ]);
