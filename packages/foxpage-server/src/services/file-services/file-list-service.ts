...
import { CONDITION_TYPE, DELIVERY_REFERENCE, FUNCTION_TYPE, VARIABLE_TYPE } from '../../../config/constant';
...
export class FileListService extends FileServiceAbstract {
  ...
  async getFileAssocInfo(fileList: File[], options?: { type: string }): Promise<FileAssoc[]> {
    ...
    let [fileIds, folderIds, creatorIds] = [[], [], []] as string[][];
    ...
    if ([VARIABLE_TYPE, CONDITION_TYPE, FUNCTION_TYPE].indexOf(fileType) !== -1) {
