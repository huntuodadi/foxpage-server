...
export class GetAppComponentListByNameVersion extends BaseController {
  ...
  async index2(@Body() params: AppNameVersionPackagesReq): Promise<ResData<NameVersionPackage[]>> {
    try {
      ...
      for (const content of contentList as any[]) {
        if (!content.package) {
          content.package = {};
        }

        // Exclude non-specified types of component data
        if (params.type && params.type.indexOf(content?.package?.type) === -1) {
          continue;
        }
