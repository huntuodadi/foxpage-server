...
import { ResData, Creator } from '../../types/index-types';
...
export class GetStoreFileGoodsList extends BaseController {
  ...
  async index(@Body() params: GetFileListReq): Promise<ResData<GoodsWithAppInfo>> {
    try {
      ...
      if (goodsList.length > 0) {
        ...
        const userIds = _.uniq(_.map(goodsDetailList, 'creator')) as string[];
        ...
        goodsList.forEach((goods) => {
          if (goods.details) {
            ...
            goods.creator = userObject[goods.details.creator as string] || '';
          }
        });
      }
