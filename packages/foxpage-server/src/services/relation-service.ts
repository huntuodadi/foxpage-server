...
import { PRE_RELATION } from '../../config/constant';
...
export class RelationService extends RelationServiceAbstract {
   ...
   * @returns Promise
   */
  async saveRelations(contentId: string, versionNumber: number, relations: any): Promise<void> {
    const contentRelations = Object.keys(relations).map((key) => {
      return { useContentId: relations[key].id as string };
    });
    ...
    if (existRelations) {
      this.updateDetailQuery(existRelations.id, { relation: contentRelations });
    } else {
      this.addDetailQuery({
        id: generationId(PRE_RELATION),
        contentId,
        versionNumber,
        relation: contentRelations,
        deleted: false,
        creator: Service.userSingleton.getInstance().getCurrentUserBase().id || '',
      });
    }
  }
  ...
  getInvalidRelationIdVersion(
    ...
    const versionObject = _.keyBy(versionList, (version) => version.contentId + '_' + version.version);
    idVersions.forEach((version) => {
      if (!versionObject[version.contentId + '_' + version.version]) {
