...
import { LOG_CATEGORY_ORGANIZATION, LOG_CREATE, PRE_APP } from '../../config/constant';
...
import { PageList } from '../types/index-types';
...
export class ApplicationService extends ApplicationServiceAbstract {
   ...
   */
  public static getInstance(newInstance: boolean = false): ApplicationService {
    if (newInstance) {
  ...
  /**
   * Set the application ID of the current request
   ...
   * @returns Content
   */
  create(params: Partial<Application>): Application {
    const appDetail: Application = {
      id: params.id || generationId(PRE_APP),
      intro: params.intro || '',
      slug: params.slug || '',
    ...
    this.addDetailQuery(appDetail);

    Service.logSingleton.getInstance().addLogData({
      action: LOG_CREATE,
      category: { type: LOG_CATEGORY_ORGANIZATION, id: params.organizationId },
      content: { id: appDetail.id, after: appDetail },
    });
    return appDetail;
  }

  /**
   * Get the application ID of the current request
   * @returns string
   */
  ...
  /**
   * Get the application organization ID of the current request
   * @returns string
   */
  ...
  async getPageListWithOrgInfo(params: AppSearch): Promise<PageList<AppOrgInfo>> {
    ...
    return {
      pageInfo: { page: params.page as number, size: params.size as number, total: total },
      data: appOrgList,
    };
  }
  ...
  async getPageList(params: AppSearch): Promise<PageList<AppInfo>> {
    ...
    if (appList.length > 0) {
      const userBase = await Service.userSingleton.getInstance().getDetailByIds(_.map(appList, 'creator'));
