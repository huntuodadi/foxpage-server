  ...
  return {
    ...
    visitor: {
      ...
      storageObjectUploadByPath: async (
        ...
        const contentPath = __dirname + '/' + ZIP_FOLDER + '/' + path;
      ...
      storageDownloadFolders: async (
        ...
        const basePath: string = __dirname + '/' + DOWNLOAD_FOLDER;
        try {
        ...
        const tmpTarget = [basePath + '/' + target, new Date().getTime()].join('_');

        filterAndCreateFolders(files, tmpTarget, prefix);

        let fileList = await bulkDownloadFiles(s3Client, files, options?.bucketName || bucketName, prefix, tmpTarget);
        let failedList = _.filter(fileList, (file) => file.code === 1);

        if (failedList.length > 0) {
          // retry again
          const failedFileList = _.map(failedList, 'objectName');
          fileList = await bulkDownloadFiles(s3Client, failedFileList, options?.bucketName || bucketName, prefix, tmpTarget);
        }
      ...
      storageZipFolders: (source: string, target: string): Promise<Record<string, any>> => {
        const basePath: string = __dirname + '/' + ZIP_FOLDER;
        try {
          fs.accessSync(basePath);
        } catch (err) {
          fs.mkdirSync(basePath);
        ...
        return new Promise(async (resolve, reject) => {
          compressing.zip
            .compressDir(__dirname + '/' + DOWNLOAD_FOLDER + '/' + source, basePath + '/' + target)
            .then((data) => {
      ...
      storageZipFileContent(target: string): Buffer {
        return fs.readFileSync(__dirname + '/' + ZIP_FOLDER + '/' + target);
      },
    },
    options: {},
  };
