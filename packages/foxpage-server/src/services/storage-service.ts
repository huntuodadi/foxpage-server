...
export class StorageService {
  ...
  async getList(prefix: string, options?: { bucket?:string, maxKeys: number }): Promise<StorageListRes[]> {
    return this.plugins.storageObjectList(prefix, { bucketName: options?.bucket||'', maxKeys: options?.maxKeys || 0 });
  }
  ...
  async downloads(prefix: string): Promise<any> {
    ...
    if (downloadPath.code === 0) {
      ...
      const zipResult = await this.plugins.storageZipFolders(downloadPath.data as string, prefixPath);
      if (zipResult.code === 0) {
  ...
  async organizeUpload(params: {
    ...
    const downloadResult = await this.plugins.storageDownloadFolders(prefix, folderPrefix, { bucketName: params?.bucket || ''});
    if (!downloadResult || downloadResult.code !== 0) {
      // Object download fail
      return { code: 2, data: downloadResult.data as string[] };
    }

    // Zip locale objects
    const zipResult = await this.plugins.storageZipFolders(
      downloadResult.data as string,
