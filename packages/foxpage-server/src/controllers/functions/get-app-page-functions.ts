  ...
  async index(@QueryParams() params: AppTypeFilesReq): Promise<ResData<ContentInfo[]>> {
    try {
      const typePageParams: AppTypeFileParams = Object.assign(
        { type: FUNCTION_TYPE, deleted: false },
        params,
      );
      ...
      if (result?.list.length > 0) {
        fileList = await this.service.file.list.getFileAssocInfo(result.list, { type: FUNCTION_TYPE });
      }
      ...
      return Response.error(err, i18n.function.getAppPageFunctionFailed);
    }
  }
}
